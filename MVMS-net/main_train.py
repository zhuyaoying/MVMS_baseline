...
def save_checkpoint(best_auc, model, optimizer, epoch):
    ...
    torch.save({
        ...
        'best_auc': best_auc,
    }, os.path.join('./CPSC_RESULTS', config.model_name, config.model_name + '_checkpoint_best.pth'))
...
def train(config=config):
    ...
    if not os.path.isdir(os.path.join('./CPSC_RESULTS', config.model_name)):
        os.makedirs(os.path.join('./CPSC_RESULTS', config.model_name), exist_ok=True)
